...
class kabagController extends Controller
{
    ...
    public function printRkb(Request $request)
    {
        ...
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."' and (e_rkb_approve.cancel_section = 'KABAG' or e_rkb_cancel.cancel_by_section = '".$request->USER."')");
        }else if($request->STATUS=="0"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and (e_rkb_header.dept = '".$_SESSION['department']."' and e_rkb_approve.disetujui = '0' and ( e_rkb_approve.cancel_section IS NULL and e_rkb_cancel.cancel_by_section IS NULL ))");
        }else if($request->STATUS=="1"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."'  and e_rkb_approve.disetujui = '1' and ( e_rkb_approve.cancel_section !='KABAG' or e_rkb_cancel.cancel_by_section IS NULL  )");
        }
        }
       else if($request->USER=="KTT"){  
        if($request->STATUS=="cancel"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."' and (e_rkb_approve.cancel_section = 'KTT' or e_rkb_cancel.cancel_by_section = '".$request->USER."')");
        }else if($request->STATUS=="0"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and (e_rkb_header.dept = '".$_SESSION['department']."' and e_rkb_approve.diketahui = '0' and (e_rkb_cancel.cancel_by_section IS NULL and e_rkb_approve.cancel_section IS NULL ))");
        }else if($request->STATUS=="1"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."'  and e_rkb_approve.diketahui = '1' and (e_rkb_cancel.cancel_by_section IS NULL and e_rkb_approve.cancel_section IS NULL ) ");
        }
        }else{
        if($request->STATUS=="cancel"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."' and (e_rkb_approve.cancel_section IS NOT NULL or e_rkb_cancel.cancel_by_section IS NOT NULL)");
        }else if($request->STATUS=="0"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and (e_rkb_header.dept = '".$_SESSION['department']."' and (e_rkb_approve.disetujui = '0' or e_rkb_approve.diketahui = '0') and (e_rkb_cancel.cancel_by_section IS NULL and e_rkb_approve.cancel_section IS NULL ))");

        }else if($request->STATUS=="1"){
        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."' and e_rkb_cancel.cancel_by_section IS NULL and (e_rkb_approve.disetujui = '1' or e_rkb_approve.diketahui = '1')");
        }else{

        $rkb = $query->whereRaw("(e_rkb_header.tgl_order BETWEEN '".$startDate."' and '".$endDate."') and e_rkb_header.dept = '".$_SESSION['department']."'");   
        }
        ...
        $row = $rkb->orderBy("e_rkb_detail.id","desc")
                    ->get();
        return view('page.kabag.printRkb',["rkb"=>$row,"getUser"=>$this->user]);
    }
    public function rkbPrint(Request $request)
    {
        ...
        $row = $rkb->orderBy("e_rkb_detail.id","desc")
                    ->get();
        return view('page.kabag.rkbPrint',["rkb"=>$row,"getUser"=>$this->user]);
    }
    ...
    public function rkb(Request $request)
    {
        ...
        if(isset($request->close_rkb)){
            ...
            $sql = DB::table('e_rkb_header') 
                    ...
                    ->whereRaw("e_rkb_header.dept ='".$_SESSION['department']."'");
            if($request->search){
        ...
        return view('page.kabag.v2.rkb',["rkb"=>$rkb,"getUser"=>$this->user,"kabag"=>$kabag]); 
    }
    public function compose(Request $request,$no_rkb,$user_to)
    {
        if(!isset($_SESSION['username'])) return redirect('/');
        return view('page.v2.compose',["getUser"=>$this->user,"no_rkb"=>$no_rkb,"user_to"=>$user_to]); 
    }
