...
<div class="right_col" role="main">
  <div class="col-lg-12">
    ...
    <a href="{{url('/pln/monitoring/form/barging')}}">Form Barging PLN</a>
    <br>
    <br>
  </div>
  <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
              <div class="x_panel">
                <div class="x_title">
                  <h2>Form Monitoring Barging PLN</h2>
...
<form class="form-horizontal" method="post" action="">
  ...
  <div class="form-group">
    <div class="col-lg-offset-3 col-lg-3">
      ...
      <a href="{{url('/pln/form/barging')}}" class="btn btn-danger">Batal</a>
    </div>
  </div>
</form>
  </div>
...
<div class="row">
  <div class="col-lg-12">
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        <?php
        $daily =  Illuminate\Support\Facades\DB::table("monitoring_pln.barging")->orderBy("tgl","desc")->paginate(7);
        ...
        <tr style="background-color: red;color: white;">
          ...
          <td>
            @if(isset($_GET['page']))
            <a style="color: white;" href="{{url('/pln/form/barging/q-'.bin2hex($row->tgl))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a style="color: white;" href="{{url('/pln/form/barging/q-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a style="color: white;" href="{{url('/pln/form/barging/undo-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-undo"></i></a></td>
        </tr>
        @else
        <tr>
          <td>{{date("d F Y",strtotime($row->tgl))}}</td>
          ...
          <td>
            @if(isset($_GET['page']))
            <a href="{{url('/pln/form/barging/q-'.bin2hex($row->tgl))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a href="{{url('/pln/form/barging/q-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a href="{{url('/pln/form/barging/delete-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-trash"></i></a></td>
        </tr>
...
<script>
  ...
  $.ajax({
    type:"GET",
    url:"{{url('/pln/form/barging/check')}}",
