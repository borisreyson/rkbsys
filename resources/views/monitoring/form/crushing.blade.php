...
<div class="right_col" role="main">
  <div class="col-lg-12">
    ...
    <a href="{{url('/monitoring/form/crushing')}}">Form CRUSHING</a>
    <br>
    <br>
  </div>
  <div class="row">
...
<form class="form-horizontal" method="post" action="">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-5">
      ...
      @else
      <input type="text" name="plan_daily" class="form-control" required="required" placeholder="Plan Daily" value="{{number_format($daily->cr_daily_planing,3)}}">
      @endif
    </div>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-5">
      ...
      @else
      <input type="text" name="mtd_plan" class="form-control" required="required" placeholder="Mtd Plan" value="{{number_format($daily->cr_mtd_planing,3)}}">
      @endif
    </div>
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
    <div class="col-lg-offset-3 col-lg-6">
      ...
      <a href="{{url('/monitoring/form/crushing')}}" class="btn btn-danger">Batal</a>  
    </div>
  </div>
</form>
  </div>
...
<form class="form-horizontal" method="post" action="{{url('/import/abp/crushing')}}" enctype="multipart/form-data">
  {{csrf_field()}}
  <div class="form-group">
    ...
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        <tr style="background-color: red;color: white;">
          ...
          <td>
            @if($row->mtd_actual!=0 && $row->mtd_plan!=0)
            {{number_format($row->mtd_actual/$row->mtd_plan,2)*100}} %
            @elseif($row->mtd_actual!=0 && $row->mtd_plan==0)
            {{number_format(100)}} %
            @else
            {{number_format(0)}} %
            @endif
          ...
          <td>
            @if(isset($_GET['page']))
            <a style="color: white;" href="{{url('/monitoring/form/crushing/q-'.bin2hex($row->tgl))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a style="color: white;" href="{{url('/monitoring/form/crushing/q-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a style="color: white;" href="{{url('/monitoring/form/crushing/undo-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-undo"></i></a></td>
        </tr>
        @else
        <tr>
          <td>{{date("d F Y",strtotime($row->tgl))}}</td>
          ...
          <td>
            @if(isset($_GET['page']))
            <a href="{{url('/monitoring/form/crushing/q-'.bin2hex($row->tgl))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a href="{{url('/monitoring/form/crushing/q-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a href="{{url('/monitoring/form/crushing/delete-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-trash"></i></a></td>
        </tr>
