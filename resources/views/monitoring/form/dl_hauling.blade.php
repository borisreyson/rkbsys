...
<div class="right_col" role="main">
	<div class="col-lg-12">
		...
		<a href="{{url('/monitoring/form/boat')}}">Form Delay Hauling</a>
		<br>
		<br>
	</div>
	<div class="row">
...
<form class="form-horizontal" method="post" action="">
	...
	<div class="form-group">
		...
		<div class="col-lg-1">
			@if(isset($edit) == 'true')
			<input type="text" name="start" class="form-control" id="dl_daily" required="required" placeholder="Delay Daily" value="{{$HLDelay->start}}">
			@else
			<input type="text" name="start" class="form-control" id="dl_daily" required="required" value="{{date('H:i')}}">
			@endif
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-lg-1">
			@if(isset($edit) == 'true')
			<input type="text" name="finish" id="dl_mtd" class="form-control" required="required" placeholder="Delay Mtd" value="{{$HLDelay->finish}}">
			@else
			<input type="text" name="finish" id="dl_mtd" class="form-control" required="required" value="{{date('H:i')}}">
			@endif
		</div>
	</div>
	<div class="form-group">
	...
	<div class="form-group" id="dynamicForm" @if(isset($HLDelay->dynamicField)=='') hidden="hidden" @endif>
		<label class="control-label col-lg-3 myLabel" id="myLabel" @if(isset($HLDelay->dynamicField)=='' || ($z==0||$z==2)) hidden="hidden" @endif>Lokasi</label>
		<label class="control-label col-lg-3 myLabel" id="myLabel" @if(isset($HLDelay->dynamicField)=='' || $z==1) hidden="hidden" @endif>Penyebab Utama</label>
		<div class="col-lg-3">
			@if(isset($edit) == "true")
			<input type="text" name="dynamicField" class="form-control" placeholder="" value="{{$HLDelay->dynamicField}}" >
			@else
	...
	<div class="form-group">
		<div class="col-lg-offset-3 col-lg-3">
			...
			<a href="{{url('/monitoring/form/delay/hauling')}}" class="btn btn-danger">Batal</a>  
		</div>
	</div>
</form>
	</div>
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				<tr style="background-color: red;color: white;">
					...
					<td>{{$row->dynamicField}}</td>
					<td>{{$row->keterangan}}</td>
					<td><a style="color: white;" href="{{url('/monitoring/form/delay/hauling/q-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a><a style="color: white;" href="{{url('/monitoring/form/delay/hauling/undo-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-undo"></i></a></td>
				</tr>
				@else
				<tr>
					<td>{{date("d F Y",strtotime($row->tgl))}}</td>
					...
					<td>{{$row->type_delay}}</td>
					<td>
						@if($row->dynamicField==""||$row->dynamicField==null)
							-
						@else
							{{$row->dynamicField}}
						@endif
					...
					<td>{{$row->keterangan}}</td>
					<td><a href="{{url('/monitoring/form/delay/hauling/q-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a><a href="{{url('/monitoring/form/delay/hauling/delete-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-trash"></i></a></td>
				</tr>
