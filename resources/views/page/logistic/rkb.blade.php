...
@if(isset($_GET['expired'])!="")
  <a href="{{url('/logistic/rkb')}}" class="btn btn-default">RKB</a>
@else
  <!--<a href="{{url('/logistic/rkb?expired=notnull')}}" class="btn btn-danger"> RKB EXPIRED</a>-->
@endif
@if(isset($_GET['close_rkb'])!="")
<a href="{{url('/logistic/rkb')}}" class="btn btn-default">RKB</a>
@else
<a href="{{url('/logistic/rkb?close_rkb=all')}}" class="btn btn-default">RKB Close</a>
@endif
</div>
</div>
<div class="col-md-6 col-sm-12 col-xs-12">
...
@php
  $user_cancel = Illuminate\Support\Facades\DB::table("user_login")->where("username",$value->cancel_user)->first();
@endphp
<label class="label label-danger" type="button" style="cursor: pointer;cursor: hand;" title="Remarks: {{$value->remark_cancel}}" data-toggle="tooltip">Cancel By {{$user_cancel->nama_lengkap}}</label>@if(isset($_GET['close_rkb']))
<br>
<br>
@endif
@else
...
<div class="col-sm-8">{{$value->myStatus}}</div>
        </div>
      </div>
      </div>
    </div>  
    ...
    <div class="col-xs-12" style="border-top: solid thin #000;padding-top: 5px;">
      <div class="row">
        <div class="col-sm-3">
          <a href="#" class="qty btn btn-xs btn-primary" data-id="{{$value->no_rkb}}" id="qty" data-toggle="modal" data-target="#myModal">Edit Quantity</a>
        ...
        <div class="col-sm-3">
        @if($value->cancel_user==NULL)
        <a href="#" no-rkb="{{bin2hex($value->no_rkb)}}" id="close_rkb" class="btn btn-xs btn-danger close_rkb" data-toggle="modal" data-target="#myModal">Close RKB</a>
        @else
        <a href="#" no-rkb="{{bin2hex($value->no_rkb)}}" id="close_rkb_cancel" class="btn btn-xs btn-danger close_rkb" data-toggle="modal" data-target="#myModal">Close RKB</a>
        @endif
        </div>
        @endif
...
<script>
  ...
  $("a[id=close_rkb_cancel]").on("click",function(){
      ...
      $.ajax({
        type:"POST",
        url:"{{url('/api/rkb/close.rkb.cancel')}}",
        data:{no_rkb:no_rkb},
        beforeSend:function(){
  ...
  $("a[id=details]").on("click",function(){
      ...
      $.ajax({
        type:"POST",
        url:"{{url('/logistic/rkb/detail.py')}}",
