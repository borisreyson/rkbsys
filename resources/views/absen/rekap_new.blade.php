    ...
    @if(count($dbKaryawan)>0)
      ...
      <tr class="text-center">
       ...
       $start = strtotime(date("Y-m-d"));
        ...
        while($start <= $end)
              {
                $rosterMasuk = $rosterTB->where("nik",$v->nik)->where("tanggal",date("Y-m-d",$start))->first();
            ...
            <td class="long" @if($masuk->flag=='1') style="color:white; background-color:red;" @else style=" color:black;" @endif >
              <?php
                    ...
                    if(isset($rosterMasuk->masuk)){
                      if(strtotime("+10 Minutes",strtotime($rosterMasuk->masuk))>strtotime($masuk->jam)){
                        echo "<b>".$masuk->jam."</b>";
                      }else{
                        if($rosterMasuk->kode_jam=="OFF"){
            ...
            <td class="long">
              <?php
            if(isset($rosterMasuk->kode_jam)){
              if($rosterMasuk->kode_jam=="OFF"){
                echo "<b style='color:red'>".$rosterMasuk->kode_jam."</b>";
              }else{
                echo "<b>-</b>";
              }
            }else{
      ...
      <tr class="text-center">
        ...
        while($start <= $end)
              {
                $dt = date("Y-m-d",strtotime("+1 days",$start));
                    $masukCek = $ceklogTB->where('nik',$v->nik)->where("status","Masuk")->where("tanggal",date("Y-m-d",$start))->first();

                    $rMasuk = $rosterTB->where("nik",$v->nik)->where("tanggal",date("Y-m-d",$start))->first();

                    if(strtotime($rMasuk->masuk) > strtotime($rMasuk->pulang)){
                      $pulang = $ceklogTB->where('nik',$v->nik)->where("status","Pulang")->where("tanggal",$dt)->first();
                      $m = ($masukCek!=null)? $masukCek->jam : "nol";
                      $p = ($pulang!=null)? $pulang->jam : "nol";
                      $o = ($pulang!=null)? strtotime($pulang->jam) : null;
                      // echo date("Y-m-d",$start)." ( ".$m." - ".$p." ) <br>";
                    }elseif (strtotime($rMasuk->masuk) < strtotime($rMasuk->pulang)) {
                      $pulang = $ceklogTB->where('nik',$v->nik)->where("status","Pulang")->where("tanggal",date("Y-m-d",$start))->first();
                      $m = ($masukCek!=null)? $masukCek->jam : "nol";
                      $p = ($pulang!=null)? $pulang->jam : "nol";
                      // echo date("Y-m-d",$start)." ( ".$m." - ".$p." ) <br>";
                      $n = ($pulang!=null)? strtotime($pulang->jam) : null;
                    }elseif($rMasuk->kode_jam=="OFF"){
                      $pulang = $ceklogTB->where('nik',$v->nik)->where("status","Pulang")->where("tanggal",date("Y-m-d",$start))->first();
                      $m = ($masukCek!=null)? $masukCek->jam : "nol";
                      $p = ($pulang!=null)? $pulang->jam : "nol";
                      // echo date("Y-m-d",$start)." ( ".$m." - ".$p." ) :: ".$rMasuk->kode_jam." <br>";
                      $n = ($pulang!=null)? strtotime($pulang->jam) : null;
                    }

                    if($pulang!=null){
                      $rosterPulang = $rosterTB->where("nik",$pulang->nik)->where("tanggal",date("Y-m-d",$start))->first();
            ?>
            <td class="long" @if($pulang->flag=='1') style=" color:white; background-color:red;" @else style=" color:black;" @endif >
              <?php
                    if(isset($rMasuk->pulang)){
                      if(strtotime($rMasuk->pulang) < strtotime($pulang->jam)){
                        if($rMasuk->kode_jam=="OFF"){
                          echo "<b style='color:red'>".$rMasuk->kode_jam."</b><br>";
                          if(!(isset($o)===isset($n))){
                            echo "<b>".$p."</b>";
                          }
                      }else{
                        ...
                        if($rMasuk->kode_jam=="OFF"){
                          if($pulang->flag=='1'){
                            echo "<b style='color:white'>".$rMasuk->kode_jam."</b>";
                          }else{
                            echo "<b color='blue'>".$rMasuk->kode_jam."</b>";
                          }
                        }else{
                          if($pulang->flag=='1'){
                            echo "<b style='color:white'>".$p." (Belum Jam Pulang)</b>";
                          }else{
                            echo "<b >".$p." (Belum Jam Pulang) | ".$pulang->lupa_absen."</b>";
                          }
                        }
              ...
              <?php
                      ...
                      ?>
            <td class="long"><?php
            if(isset($rMasuk->kode_jam)){
              if($rMasuk->kode_jam=="OFF"){
                echo "<b style='color:red'>".$rMasuk->kode_jam."</b>";
              }else{
                echo "<b>-</b>";
              }
            }else{
