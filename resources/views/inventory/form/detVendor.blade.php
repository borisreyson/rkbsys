...
<?php foreach($detail as $k => $v){ ?>
		...
		<tr>
			...
			<input type="hidden" name="nama_supplier" value="{{($v->nama_supplier)}}">
			<td>{{$v->nama_supplier}}</td>
			<td>{{$v->nama_instansi}}</td>
			<td>{{$v->alamat}}</td>
			<td>{{$v->nmr_contact}}</td>
			<td>{{$v->category_vendor}}</td>
		</tr>
<?php } ?>
</tbody>
</table>
...
<script>
	...
	$("button[name=dipilih]").click(function() {
		...
		nama_supplier = $("input[name=nama_supplier]").eq(eq).val();
		parentData = window.opener.setVendor(nama_supplier,"{{$eq}}");
		window.close();
	})
</script>