...
<div class="right_col" role="main">
  <div class="col-lg-12">
    ...
    <a href="{{url('/mhu/monitoring/form/crushing')}}">Form CRUSHING MHU</a>
    <br>
    <br>
  </div>
  <div class="row">
...
<form class="form-horizontal" method="post" action="">
  ...
  <div class="form-group">
    <div class="col-lg-offset-3 col-lg-3">
      ...
      <a href="{{url('/mhu/form/crushing')}}" class="btn btn-danger">Batal</a>  
    </div>
  </div>
</form>
  </div>
    ...
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        <tr style="background-color: red;color: white;">
          ...
          <td>
            @if(isset($_GET['page']))
            <a style="color: white;" href="{{url('/mhu/form/crushing/q-'.bin2hex($row->tgl))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a style="color: white;" href="{{url('/mhu/form/crushing/q-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a style="color: white;" href="{{url('/mhu/form/crushing/undo-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-undo"></i></a></td>
        </tr>
        @else
        <tr>
          <td>{{date("d F Y",strtotime($row->tgl))}}</td>
          ...
          <td>
            @if(isset($_GET['page']))
            <a href="{{url('/mhu/form/crushing/q-'.bin2hex($row->tgl))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a href="{{url('/mhu/form/crushing/q-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a href="{{url('/mhu/form/crushing/delete-'.bin2hex($row->tgl))}}" class="btn btn-xs"><i class="fa fa-trash"></i></a></td>
        </tr>
...
<script>
  ...
  $.ajax({
    type:"GET",
    url:"{{url('/mhu/monitoring/form/crushing/check')}}",
