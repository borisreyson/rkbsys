...
<div class="right_col" role="main">
	<div class="col-lg-12">
		...
		<a href="{{url('/monitoring/form/boat')}}">Form BOAT PLN</a>
		<br>
		<br>
	</div>
	<div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12">
							<div class="x_panel">
								<div class="x_title">
									<h2>Form Monitoring BOAT PLN</h2>
...
<div class="row">
	<div class="col-lg-12">
		<table class="table table-bordered table-striped">
			...
			<tbody>
				<?php
			$daily = Illuminate\Support\Facades\DB::table("monitoring_pln.barge_boat")->orderBy("tgl","desc")->paginate(7);
        foreach($daily as $k =>$row){
				...
				<tr style="background-color: red;color: white;">
					...
					<td>
            @if(isset($_GET['page']))
            <a style="color: white;" href="{{url('/pln/form/boat/q-'.bin2hex($row->no))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a style="color: white;" href="{{url('/pln/form/boat/q-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a style="color: white;" href="{{url('/pln/form/boat/undo-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-undo"></i></a></td>
				</tr>
				@else
				<tr>
					<td>{{date("d F Y",strtotime($row->tgl))}}</td>
					...
					<td>
            @if(isset($_GET['page']))
            <a href="{{url('/pln/form/boat/q-'.bin2hex($row->no))}}?page={{$_GET['page']}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @else
            <a href="{{url('/pln/form/boat/q-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-pencil"></i></a>
            @endif
            <a href="{{url('/pln/form/boat/delete-'.bin2hex($row->no))}}" class="btn btn-xs"><i class="fa fa-trash"></i></a></td>
				</tr>
