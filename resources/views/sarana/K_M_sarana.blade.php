    ...
    <div class="col-lg-6 col-xs-12">
      <table>
        <tr>
          ...
          <td>&nbsp;&nbsp;&nbsp; &nbsp;</td>
          <td>
            Pemohon <font color="blue" style="font-weight: bolder;"> {{ucwords($pemohon->nama)}}</font>
          </td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          ...
          <td>
            <span>
@if(!isset($_GET['dt_expr']))
          @if($v->flag_appr==2 && $v->flag==0)
          @if($v->no_lv=="motor" || $v->no_lv=="mobil")
              <a href="" id="editMotor" data_id="{{$v->noid_out}}" class="btn btn-xs btn-warning" >Edit</a>
          @else
              <a href="" id="edit" data_id="{{$v->noid_out}}" class="btn btn-xs btn-warning" >Edit</a>
          @endif
              <a href="" id="cancel"  data_id="{{$v->noid_out}}" class="btn btn-xs btn-danger" >Cancel</a>
              @endif
@endif
            </span>
          </td>
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="col-lg-12">
            ...
            @else
          <b class="col-lg-2">No LV </b> <span class="col-lg-9"><b>:</b> {{$v->no_lv}}</span>
          @endif
          </div>
          <div class="col-lg-12">
            @if($v->no_lv=="motor" || $v->no_lv=="mobil")
...
<script>
  ...
  $("a[id=editMotor]").click(function(){
    ...
    window.open("{{url('/sarpras/sarana/keluar-masuk/Motor/edit')}}"+noid_out,"_blank","width="+screen.width+",height="+screen.height+",top=50,left=200");
    return false;
  });
  ...
  $("a[id=cancel]").click(function(){
    ...
    window.open("{{url('/sarpras/sarana/keluar-masuk/cancel')}}"+noid_out,"_blank","width="+(screen.width/2)+",height="+(screen.height/2)+",top=50,left=200");
    return false;
  });
  var eventDates = {};
  <?php
  ...
  $("button[id=newIzinForm]").click(function(){
    $.ajax({
      type:"GET",
      url :"{{url('/sarpras/sarana/form-izin-keluar')}}",
      beforeSend:function(){
        $("div[id=modal_dialog]").removeClass("modal-md").addClass("modal-lg");
      },
      success:function(res){
