...
<input type="hidden" name="noid_out" value="{{$noid_out}}" >
    <div class="item form-group">
      ...
      <div class="col-md-2 col-sm-6 col-xs-12">
        <select id="no_lv" class="form-control col-md-12 col-xs-12" data-validate-length-range="2" name="no_lv" placeholder="No Polisi" required="required" type="text" data-live-search="true">
          ...
          @if($get->no_lv=="motor")
          <option value="{{$get->no_lv}}" selected="selected">{{ucwords($get->no_lv)}} </option>
          @elseif($get->no_lv=="mobil")
          <option value="{{$get->no_lv}}" selected="selected">{{ucwords($get->no_lv)}} </option>
          @else
          <option value="{{$get->no_lv}}">{{ucwords($get->no_lv)}} </option>
          @endif
        </select>
      </div>
    </div>
    <div class="item form-group">
      ...
      <div class="col-md-2 col-sm-6 col-xs-12">
        <input id="no_pol" class="form-control col-md-12 col-xs-12" data-validate-length-range="2" name="no_pol" required="required" type="text" value="{{$get->no_pol}}">
      </div>
    </div>
    <div class="item form-group">
    ...
    <div class="item form-group">
      <label class="control-label col-md-3 col-sm-3 col-xs-12" for="driver">Merk Kendaraan <span class="required">*</span>
      </label>
      <div class="col-md-6 col-sm-6 col-xs-12">
        <input type="text"  id="driver" class="form-control col-md-12 col-xs-12" name="driver" value="{{$get->driver}}" >
      </div>
    </div>
    <div class="item form-group">
        ...
        <table class="table table-striped" id="table_penumpang">
          ...
          <tbody>
            ...
            <tr>
                  ...
                  @if($pV==$vK->nik)
                  <option value="{{$vK->nik}}" selected="selected">({{$vK->nik}}) {{ucwords($vK->nama)}} [ {{ucwords($vK->jabatan)}} ]</option>
                  @else
                  <option value="{{$vK->nik}}">({{$vK->nik}}) {{ucwords($vK->nama)}} [ {{ucwords($vK->jabatan)}} ]</option>
                  @endif
                  @endforeach
                  </select>
                  </div>
