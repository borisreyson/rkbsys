  ...
  <table class="table table-bordered text-center">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{$v->jenis_sim}}</td>
        <td>{{date("d F Y",strtotime($v->berlaku_sim))}}</td>
        <td>{{ucwords($v->kota_dikeluarkan)}}</td>
        <td>
          <button class="btn btn-xs btn-warning" id="editDriver" name="editDriver" data-id="{{bin2hex($v->no)}}" data-toggle="modal" data-target="#myModal"><i class="fa fa-pencil"></i></button>
          <a href="{{url('/sarpras/data/driver/delete-')}}{{bin2hex($v->no)}}" class="btn btn-xs btn-danger" id="deleteDriver" name="deleteDriver"><i class="fa fa-trash"></i></a>
        </td>
      </tr>
      @endforeach
      @else
